...
export class PracticeQuestionSubmitter extends Contributor {
  ...
  async seedTextToQuestion(text: string): Promise<void> {
    ...
    await this.page.waitForSelector(stateContentInputField, {state: 'visible'});
  ...
  async addMathExpressionToQuestion(): Promise<void> {
    ...
    await insertMathExpressionButton?.click();
  ...
  async addImageToQuestion(): Promise<void> {
    ...
    await this.page.waitForSelector(uploadImageButton, {state: 'visible'});
  ...
  async expectQuestionSuggestionInContributorDashboard(
    ...
    await this.page.waitForSelector(opportunityListItem, {state: 'visible'});
  ...
  async addTextInputInteractionInQuestionEditor(answer: string): Promise<void> {
    ...
    await this.page.waitForSelector(textInputField, {state: 'visible'});
  ...
  async addImageInteractionInQuestionEditor(): Promise<void> {
    ...
    await this.clickOnElementWithSelector(useTheUploadImageButton);
    await this.waitForPageToFullyLoad();
    await this.page.waitForSelector('.btn-danger', {state: 'visible'});
  ...
  async fillValueInInteractionResponseModal(
    ...
    const inputElement = await this.page.waitForSelector(`xpath=${xpath}`, {
      state: 'visible',
    });
    ...
    await this.page.waitForFunction(
      args => {
        return (
          (
            args.element as HTMLInputElement | HTMLTextAreaElement
          ).value.trim() === args.value
        );
      },
      {element: inputElement, value: value}
    );
  }
...
export let QuestionSubmitterFactory = (page: Page): PracticeQuestionSubmitter =>
  new PracticeQuestionSubmitter(page);
