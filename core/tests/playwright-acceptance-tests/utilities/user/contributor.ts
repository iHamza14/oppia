...
export class Contributor extends ExplorationEditor {
  ...
  async clickOnPaginationButton(button: 'previous' | 'next'): Promise<void> {
    ...
    await this.page.waitForFunction(
      args => {
        ...
        return element?.textContent !== args.initialContent;
      },
      {
  ...
  async navigateToTabInMyContributions(
    ...
    if (this.isViewportAtMobileWidth()) {
      ...
      const optionElement = await this.page.waitForSelector(xpath);
      ...
      const element = await this.page.waitForSelector(xpath);
      if (!element) {
      ...
      await this.page.waitForSelector(xpathActive);
    }
  }
  ...
  async selectBadgeTypeInMobileView(
    ...
    const badgeOption = await this.page.waitForSelector(buttonXPath);
    if (!badgeOption) {
  ...
  async switchToTabInContributionDashboard(
    ...
    await this.page.waitForFunction(
      ...
      [contributionTabSelector, tabName]
    );
