...
export class PracticeQuestionReviewer extends Contributor {
  ...
  /**
   * Removes the current interaction from the question editor.
   */
  async removeInteraction(): Promise<void> {
    const deleteInteractionButtonSelector = '.e2e-test-delete-interaction';
    const confirmDeleteButtonSelector = '.e2e-test-confirm-delete-interaction';

    await this.clickOnElementWithSelector(deleteInteractionButtonSelector);

    const isConfirmVisible = await this.isElementVisible(
      confirmDeleteButtonSelector
    );
    if (isConfirmVisible) {
      await this.clickOnElementWithSelector(confirmDeleteButtonSelector);
    }
  }
...
export let PracticeQuestionReviewerFactory = (
  page: Page
): PracticeQuestionReviewer => new PracticeQuestionReviewer(page);
